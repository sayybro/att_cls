...
@torch.no_grad()
def evaluate_att(dataset_file, model, attribute_classifier, postprocessors, data_loader, subject_category_id, device):
    model.eval()
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        samples = samples.to(device)
        encoder_output = model(samples)
        outputs = attribute_classifier(encoder_output)
