...
class HungarianMatcherHOI(nn.Module):
    ...
    def __init__(self, cost_obj_class: float = 1, cost_class: dict={}, cost_bbox: float = 1,
                 cost_giou: float = 1):
        super().__init__()
        self.cost_obj_class = cost_obj_class
        if 'hoi' in cost_class:
            self.cost_verb_class = cost_class['hoi']
        if 'att' in cost_class:
            self.cost_att_class = cost_class['att']
        
        self.cost_bbox = cost_bbox
        self.cost_giou = cost_giou
        assert cost_obj_class != 0 or cost_class != 0 or cost_bbox != 0 or cost_giou != 0, 'all costs cant be 0'
    ...
    def forward(self, outputs, targets, dtype):
        ...
        if dtype=='hoi':
            ...
            tgt_labels_permute = tgt_labels.permute(1, 0)
            tgt_sub_boxes = torch.cat([v['sub_boxes'] for v in targets])
            tgt_obj_boxes = torch.cat([v['obj_boxes'] for v in targets])
        elif dtype=='att':
            tgt_obj_labels = torch.cat([v['labels'] for v in targets])
            tgt_labels = torch.cat([v['pos_att_classes'] for v in targets])
            tgt_labels_permute = tgt_labels.permute(1, 0)
            tgt_obj_boxes = torch.cat([v['boxes'] for v in targets])
        
        if len(tgt_labels) != len(tgt_obj_labels):
            import pdb; pdb.set_trace()

        cost_obj_class = -out_obj_prob[:, tgt_obj_labels]

        
        cost_class = -(out_prob.matmul(tgt_labels_permute) / \
                            (tgt_labels_permute.sum(dim=0, keepdim=True) + 1e-4) + \
                            (1 - out_prob).matmul(1 - tgt_labels_permute) / \
                            ((1 - tgt_labels_permute).sum(dim=0, keepdim=True) + 1e-4)) / 2
        if dtype=='hoi':
        ...
        cost_obj_giou = -generalized_box_iou(box_cxcywh_to_xyxy(out_obj_bbox), box_cxcywh_to_xyxy(tgt_obj_boxes)) + \
                        cost_sub_giou * (tgt_obj_boxes == 0).all(dim=1).unsqueeze(0) if dtype=='hoi' else -generalized_box_iou(box_cxcywh_to_xyxy(out_obj_bbox), box_cxcywh_to_xyxy(tgt_obj_boxes))
        ...
        if dtype=='hoi':
            ...
            C = self.cost_obj_class * cost_obj_class + self.cost_att_class * cost_class + \
                self.cost_bbox * cost_bbox + self.cost_giou * cost_giou 
        C = C.view(bs, num_queries, -1).cpu()

        sizes = [len(v['obj_labels']) for v in targets] if dtype=='hoi' else [len(v['labels']) for v in targets]
        indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))]
        return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices]


class HungarianMatcherHOI_orig(nn.Module):
